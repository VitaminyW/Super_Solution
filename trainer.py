...
class Trainer_fine_tune():
    ...
    def train(self):
        ...
        self.model.train()
        timer_data, timer_model = utility.timer(), utility.timer()
        for batch, (lr, hr, _, unpaired) in enumerate(self.loader_train):
            timer_data.hold()
            timer_model.tic()
            self.optimizer.zero_grad()
            loss_primary = torch.zeros(1).to(torch.device('cpu' if self.opt.cpu else 'cuda'))
            loss_dual = torch.zeros(1).to(torch.device('cpu' if self.opt.cpu else 'cuda'))
            unpaired_loss = torch.zeros(1).to(torch.device('cpu' if self.opt.cpu else 'cuda'))
            for i in range(len(self.dual_optimizers)):
                self.dual_optimizers[i].zero_grad()
            if len(lr) > 0:
                lr, hr = self.prepare(lr, hr)
            ...
            if not isinstance(unpaired, list):
                # forward unpaired
                unpaired = unpaired.to(torch.device('cpu' if self.opt.cpu else 'cuda'))
                unpaired_sr = self.model(unpaired)
                unpaired_sr2lr = self.dual_models[0](unpaired_sr[- len(self.dual_models)])
                # unpaired loss
                unpaired_loss += self.loss(unpaired_sr2lr, unpaired)
            # compute total loss
            loss = loss_primary + self.opt.dual_weight * (loss_dual + unpaired_loss)
...
class Trainer_reconstruction():
    ...
    def reconstruction(self):
        ...
        with torch.no_grad():
            ...
            for si, s in enumerate([scale]):
                eval_psnr = 0
                tqdm_test = tqdm(self.loader_train, ncols=80)
                for _, (unpaired,filename) in enumerate(tqdm_test):
                    unpaired = unpaired.to(torch.device('cpu' if self.opt.cpu else 'cuda'))
                    sr = self.model(unpaired)
                    if isinstance(sr, list): sr = sr[-1]
                    sr = utility.quantize(sr, self.opt.rgb_range)
                    # save test results
                    if self.opt.save_results:

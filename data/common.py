...
def get_patch_unpaired(*args, patch_size=96, scale=8):
    # print('args[-1]',args[-1].shape[:2]*scale)
    th, tw = args[-1].shape[:2] # target images size
    ...
    tx = random.randrange(0, tw - tp + 1)
    ty = random.randrange(0, th - tp + 1)
    tx, ty = tx- tx % scale, ty - ty % scale
    ix, iy = tx // scale, ty // scale
    lr = args[-1][iy:iy + ip, ix:ix + ip, :]
    return lr

def set_channel(*args, n_channels=3,unpaired = False):
...
def np2Tensor(*args, rgb_range=255,unpaired = False):
    def _np2Tensor(img):
    ...
    if not unpaired:
        ...
        return _np2Tensor(args[-1])


def augment(*args, hflip=True, rot=True, unpaired=False):
